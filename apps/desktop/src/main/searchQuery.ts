...
// Ordered, de-duped queries to try in turn: the cleaned query first (keeps a mix
// name so a remix still resolves), then the same without any parenthetical (the
// bare title that usually works when the fuller query returns nothing). Falls back
...
export function buildSearchCandidates(query: string): string[] {
  ...
  const out: string[] = []
  for (const candidate of [cleaned, stripParentheticals(cleaned)]) {
