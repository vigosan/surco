...
// LoudnessReadout's shape — two labelled groups of auto-fill cells, each a field-coloured
// card with a status dot, a label line and a value line — so the measured figures swap in
// without the pills popping into an empty space.
const GROUPS = [
  { key: 'loudness', cells: 4 },
  { key: 'signal', cells: 3 },
]

export function LoudnessSkeleton(): React.JSX.Element {
  return (
    <div data-testid="loudness-skeleton" className="mt-3 space-y-3">
      {GROUPS.map((group) => (
        <div key={group.key}>
          ...
          <div className="grid gap-2 [grid-template-columns:repeat(auto-fill,minmax(6.5rem,1fr))]">
            {Array.from({ length: group.cells }, (_, i) => (
              <div
