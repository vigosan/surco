...
// In-field menu that inserts the current value of another metadata field at the
// caret ("add the year to the title" without retyping). It inserts the resolved
// value — never a live {token} — because tags must store final strings; template
// behaviour stays in the filename's RenameModal. Same interaction pattern as
// Select: backdrop click or Escape closes, arrows cycle, focus hands back to the
// input. The caret is captured on the trigger's mousedown, while the input still
// has focus (the click itself blurs it); when the input wasn't focused at all the
// value appends at the end, which matches the common intent.
  ...
  return (
    <>
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label={tr('editor.insertFromField')}
        title={tr('editor.insertFromField')}
        onMouseDown={() => {
