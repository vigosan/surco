...
// number to a collector — Discogs-style taggers write it verbatim — so a single
// letter with optional digits passes through untouched, with no disc. Anything
// else (two-letter media labels like "CD1") keeps the digits-only fallback.
export function splitPosition(position: string): { disc: string; track: string } {
  ...
  if (/^[A-Za-z]\d*$/.test(p)) return { disc: '', track: p }
