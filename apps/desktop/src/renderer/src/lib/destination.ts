...
export type Destination = 'folder' | 'appleMusic' | 'both' | 'engineDj' | 'overwrite'

export const DESTINATIONS: Destination[] = ['folder', 'appleMusic', 'both', 'engineDj', 'overwrite']
...
// destination is pinned to the output folder.
...
// Engine DJ always keeps the copy (the library row points at it). Overwrite rewrites the
// source itself, so it adds to neither library destination.
