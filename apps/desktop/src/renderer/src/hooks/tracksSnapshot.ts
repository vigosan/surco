...
// The four probe families the list reads, one entry per track, positionally aligned with
// `tracks`. Spectrum carries `fetching` too: a row with an analysis in flight shows a
// placeholder where its verdict dot will land, which the other three have no equivalent of.
...
export const SNAPSHOT_FAMILIES = ['spectrogram', 'waveform', 'waveformScan', 'beatgrid'] as const
...
// snapshot was O(N × cacheSize) — hundreds of thousands of comparisons on a big crate.
...
// Reads one track's slot across all four families out of the cache index. Shared by the full
...
// N, a probe patch reads 1 — the assertion that a sweep tick doesn't walk the whole crate.
export function readSlot(inputPath: string, at: (key: readonly unknown[]) => Query | undefined) {
  ...
  return {
    ...
    scan: at(waveformScanOptions(inputPath).queryKey)?.state.data as
      ...
      | null
      | undefined,
    grid: at(beatgridOptions(inputPath).queryKey)?.state.data as BeatgridResult | null | undefined,
  }
}

// The full positional snapshot: one pass over every track, four cache lookups each. The O(N)
...
export function buildCacheSnapshot(
  ...
  const grids: CacheSnapshot['grids'] = []
  for (const t of tracks) {
    const slot = readSlot(t.inputPath, at)
    spectra.push(slot.spectrum)
    waves.push(slot.wave)
    scans.push(slot.scan)
    grids.push(slot.grid)
  }
  return { spectra, waves, scans, grids }
}

// Re-reads one track's slot and, if any family's result actually changed, returns a new
// snapshot that reuses the three unchanged family arrays by reference and swaps one entry in
// the fourth (a shallow copy). Returns the same snapshot untouched when nothing moved, so a
// spurious event (a query updating to the same reference) never re-renders the list.
export function patchSnapshot(
  ...
  const gridChanged = prev.grids[index] !== slot.grid
  if (!spectrumChanged && !waveChanged && !scanChanged && !gridChanged) return prev
